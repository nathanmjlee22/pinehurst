...
function buildChart(){
  ...
  ORDER.forEach(g=>{
    ...
    const grad=ctx.createLinearGradient(0,0,0,240);grad.addColorStop(0,d.color+'33');grad.addColorStop(1,d.color+'00');
    datasets.push({label:d.name,data:pts,borderColor:d.color,backgroundColor:grad,borderWidth:2.5,fill:true,tension:0.3,pointRadius:filtered.length>60?0:3,pointHoverRadius:7,pointBackgroundColor:d.color,pointBorderColor:'#ffffff',pointBorderWidth:2,pointHoverBackgroundColor:'#ffffff',pointHoverBorderColor:d.color,pointHoverBorderWidth:2});
    const rpts=d.rounds.map(r=>({x:r.date,y:r.hi,name:d.name,course:r.course,score:r.score,series:'rounds'}));
    datasets.push({label:d.name+' (rounds)',data:rpts,borderColor:d.color,backgroundColor:'transparent',borderWidth:1.5,borderDash:[5,4],fill:false,tension:0.3,pointRadius:3,pointHoverRadius:6,pointBackgroundColor:d.color+'bb',pointBorderColor:'#ffffff',pointBorderWidth:1,pointHoverBackgroundColor:'#ffffff',pointHoverBorderColor:d.color,pointHoverBorderWidth:2});
  });
