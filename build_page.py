...
.t-player{display:flex;justify-content:space-between;align-items:center;gap:5px;width:100%;padding:2px 0}
.t-player+.t-player{border-top:1px solid rgba(0,0,0,.05)}
.t-pname{font-size:12px;font-weight:700;color:#0d1a10}
.team-btn.won .t-pname{color:#0a3318}
.t-phi{font-size:11px;font-weight:700;color:var(--dim);background:rgba(0,0,0,.05);border-radius:4px;padding:1px 5px;flex-shrink:0}
...
.tee-row{display:flex;align-items:center;gap:4px;margin-top:3px;margin-bottom:5px;flex-wrap:wrap}
.tee-btn{padding:2px 6px;border-radius:5px;border:1.5px solid;font-family:inherit;font-size:10px;font-weight:800;cursor:pointer;transition:all .12s;line-height:1.4}
...
.ch-badge{font-size:10px;font-weight:700;color:var(--dim);padding:1px 4px;background:rgba(0,0,0,.05);border-radius:4px}
.stk-badge{font-size:10px;font-weight:800;color:#fff;background:#0a3318;border-radius:4px;padding:1px 5px}
.stk-badge.scratch{background:var(--dim);color:#fff}
...
function renderSBRows(){
  ...
  rows.innerHTML=MATCHUPS[sbRound].map((m,i)=>{
    ...
    function playerHTML(name){
      ...
      const stkStr=stk!==null?(stk===0?'<span class="stk-badge scratch">scratch</span>':`<span class="stk-badge">+${stk}</span>`):'';
      return `<div class="t-player">
        <span class="t-pname">${name}</span>
        <span class="t-phi">${hi!==null?hi.toFixed(1):'—'}</span>
      </div>
      <div class="tee-row" data-player="${name}" data-round="${sbRound}">
