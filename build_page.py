...
function playerHI(name){const g=Object.values(G).find(x=>x.name.split(' ')[0]===name);return g?parseFloat(g.current).toFixed(1):'—';}
...
function renderSBRows(){
  ...
  rows.innerHTML=MATCHUPS[sbRound].map((m,i)=>{
    ...
    const lPlayers=m.left.map(n=>`<div class="t-player"><span class="t-pname">${n}</span><span class="t-phi">${playerHI(n)}</span></div>`).join('');
    const rPlayers=m.right.map(n=>`<div class="t-player"><span class="t-pname">${n}</span><span class="t-phi">${playerHI(n)}</span></div>`).join('');
